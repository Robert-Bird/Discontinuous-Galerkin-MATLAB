function Er=Error_calc_jump_in_displacement_flux(coord,etpl,etpl_face,ed,u,E,v)
% Error for the jump in displacement in internal faces
%
% Input(s):
...
for fn = 1:n_faces                                                         % Surface integral loop
    if el_n(fn)~=0                                                         % If internal face
        if etpl.tree(el_p(fn),1)==1 && etpl.tree(el_n(fn),1)==1
            ...
            [~,h_small] = face_flag(etpl_face(fn,:),co_pve,co_nve);        % Lenght of positive
            ...
            for gp = 1:ngp                                                 % Gauss point loop
                ...
                [~,Ls]=pen_func(co_pve,co_nve,1,pos_loc_p,neg_loc_p,E,v,nx(fn),ny(fn));
                u_n=-un'*[nx(fn);ny(fn)];                                  % un multiplyed its normal
                u2=(u_p+u_n)^2;
                max_p=max([neg_loc_p,pos_loc_p]);                          % Larger polynomial order
                Er(el_p(fn))=Er(el_p(fn))+(max_p^3)*0.5*(Ls^2)*u2*W/h_small; % Positive error summation
                Er(el_n(fn))=Er(el_n(fn))+(max_p^3)*0.5*(Ls^2)*u2*W/h_small; % Negative error summation
            end
        end
    end
end
